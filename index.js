...
if (!DISCORD_TOKEN || !RIOT_API_KEY || !ALERT_CHANNEL_ID) {
  console.error("Missing .env values. Need DISCORD_TOKEN, RIOT_API_KEY, ALERT_CHANNEL_ID");
  process.exit(1);
}
...
if (!hasStackMatchRow(gameId)) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      { name: "Queue", value: "Ranked Flex (440)", inline: true },
      { name: "Game ID", value: String(gameId), inline: true },
      { name: "Note", value: "Posting result after the match ends.", inline: false }
    )
    .setTimestamp();

  await postEmbed(client, embed);
}
  ...
  for (const row of active) {
    ...
    const lines = match.info.participants
      .filter(p => rosterPuuids.has(p.puuid) && p.teamId === teamId)
      .map(p => `• **${puuidToDisplay.get(p.puuid) || p.summonerName}** — ${p.championName} — ${p.kills}/${p.deaths}/${p.assists}`);

    const embed = new EmbedBuilder()
      .setTitle(`Flex stack finished: ${win ? "WIN" : "LOSS"}`)
      .setDescription(lines.join("\n") || "Could not match roster players in participants.")
      .addFields(
        { name: "Stack size", value: String(row.stack_size), inline: true },
        { name: "All-time stack record", value: `${record.wins}-${record.losses}`, inline: true },
